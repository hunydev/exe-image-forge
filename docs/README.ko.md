...
- `exe-image-forge/base` — node/npm/npx, python, uv/uvx, go, gh, codex, claude, gemini + systemd/exeuntu 호환 레이어
- `exe-image-forge/dev` — base + 로그인된 자격증명을 구운 이미지
- 발급 시 `LABEL`만 다른 얇은 레이어를 새 태그로 push → 태그별 digest가 고유하므로 만료 시 개별 삭제 가능
- 만료되면 태그·토큰 경로 삭제, blob은 매일 `exe-image-forge-gc.timer`가 회수
...
4. **패스키 (WebAuthn)** — 암호 외에 패스키로도 로그인할 수 있습니다. 등록은 관리 페이지에서 하고,
   ...
   패스키는 **도메인에 고정**되므로 `image-forge.exe.xyz` 와 `images.example.com` 각각 등록해야 합니다
...
5. **bake 버튼** — 웹에서 `exe-image-forge/dev:latest` 를 굽고 로그를 실시간으로 스트리밍합니다.
...
```sh
exe-image-forge build                  # 베이스 이미지 빌드
exe-image-forge auth gh                # 이미지 안에서 로그인 (또는 exe-image-forge import)
exe-image-forge status                 # 자격증명 확인
exe-image-forge bake                   # exe-image-forge/dev:latest 생성
```

자격증명은 `/var/lib/exe-image-forge/authhome` 에 보존됩니다. 이미지를 재빌드해도 유지되고, `bake` 할 때만 이미지에 들어갑니다.

### 랩톱에서 이미 로그인돼 있다면

```sh
exe-image-forge export-hint    # 랩톱에서 실행할 tar 명령을 출력
```
...
| 명령 | 설명 |
| --- | --- |
| `exe-image-forge build` | 베이스 이미지 재빌드 |
| `exe-image-forge auth {gh\|codex\|claude\|claude-token\|gemini\|all}` | 이미지 안에서 로그인 |
| `exe-image-forge import [file]` | tar 로 자격증명 반입 (기본 stdin) |
| `exe-image-forge export-hint` | 랩톱에서 쓸 export 명령 출력 |
| `exe-image-forge shell` | 영속 HOME 을 붙인 대화형 셸 |
| `exe-image-forge status` | 자격증명 존재 여부 |
| `exe-image-forge bake` | `exe-image-forge/dev:latest` 생성 |
| `exe-image-forge verify [img]` | 툴/인증 점검 |
| `exe-image-forge password` | 웹 비밀번호 변경 |
| `exe-image-forge token [tok]` | 랩톱 `docker pull` 용 VM 토큰 저장 |
| `exe-image-forge vend-build` | vend 서비스 재빌드·재시작 |
| `exe-image-forge grants` | 활성 grant 목록 |
| `exe-image-forge gc` | registry/도커 가비지 수거 |

## HTTPS 관련 주의

exe.dev 프록시는 HTTPS 만 노출하며 기본이 **인증 게이트(private)** 입니다.

- 공식 private registry 문서처럼 먼저 `ssh exe.dev share port <VM이름> 8000` 으로 프록시 포트를
  지정합니다. 프록시는 기본적으로 private 이며, 공개가 필요한 경우에만
  `ssh exe.dev share set-public <VM이름>` 을 사용하세요.
...
- 랩톱에서 직접 `docker pull` 하려면 private 상태에서도 VM 토큰을 basic-auth 비밀번호로 쓸 수 있습니다:
  `ssh exe.dev ssh-key generate-api-key --vm=image-forge --label=registry` → `exe-image-forge token <TOKEN>`

## 보안

- 비밀번호는 PBKDF2-HMAC-SHA256 (210k iters, 랜덤 salt) 로 `/etc/exe-image-forge/config.json` 에 저장
...
- 코드: `/home/exedev/exe-image-forge`
- 설정: `/etc/exe-image-forge/config.json`
- 상태: `/var/lib/exe-image-forge/{grants.json,authhome,registry}`
- 서비스: `exe-image-forge-vend.service`, `exe-image-forge-gc.timer`, `registry` (docker)
...
**이미지 자체** — 이 VM 의 `exe-image-forge-refresh.timer` 가 매주 일요일 `exe-image-forge refresh` 를 돌려
base 를 다시 빌드하고 (자격증명이 있으면) dev 를 다시 굽습니다. 자격증명은 이미지 밖
(`/var/lib/exe-image-forge/authhome`)에 있으므로 무인 실행이 안전합니다.

`update-ai-clis` 설계:

- 도구별로 독립 실행 — 한 곳의 네트워크 오류가 나머지를 막지 않습니다 (`set -e` 를 쓰지 않음)
- 새 바이너리를 임시 위치에 받아 `--version` 이 도는지 확인한 뒤에야 교체 (중단되어도 실행 불가 상태가 되지 않음)
- 결과를 `/etc/ai-cli-versions.json` 에 기록 → 관리 페이지 "도구 버전" 카드와 `exe-image-forge versions` 가 이걸 읽습니다
...
```
exe-image-forge context [variant]   # AI CLI 가 읽게 될 내용 확인
```
...
관리 터미널과 `exe-image-forge auth` 는 **`exe-image-forge/base:go-gemini`** 에서 실행됩니다.
...
| 리포지토리 | 내용 |
|---|---|
| `exe-image-forge/dev` | 도구 + **로그인된 자격증명** ← 이걸 받으세요 |
| `exe-image-forge/base` | 도구만. 자격증명 없음 (bake 의 재료) |

`exe-image-forge/base` 를 받으면 `gh auth status`, codex, claude, gemini 모두 **로그인이 안 된 상태**입니다.
버그가 아니라 설계입니다 — base 는 자격증명이 들어가기 전 단계입니다.

예전에는 홈 화면 드롭다운에 `exe-image-forge/base` 가 먼저 떠서 기본 선택이었고, 이름만으로는 차이를
알 수 없었습니다. 지금은 `exe-image-forge/dev` 가 기본이고 두 항목 모두 라벨로 구분됩니다.

bake 는 끝나고 실제로 로그인이 넘어갔는지 확인합니다:

```
==> verified: all four logins survive into exe-image-forge/dev:min
```
...
`COMPRESSION=gzip exe-image-forge build` 로 되돌릴 수 있습니다.
